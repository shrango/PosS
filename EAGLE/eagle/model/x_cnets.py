...
class LlamaCrossAttention(nn.Module):
    ...
    def forward(
            ...
            kv_hidden_states: torch.Tensor = None,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        bsz, q_len, _ = hidden_states.size()
        kv_len = kv_hidden_states.size()[1]
        ...
        if self.config.pretraining_tp > 1:
            ...
            key_states = [F.linear(kv_hidden_states, key_slices[i]) for i in range(self.config.pretraining_tp)]
            key_states = torch.cat(key_states, dim=-1)

            value_states = [F.linear(kv_hidden_states, value_slices[i]) for i in range(self.config.pretraining_tp)]
        ...
        # 可能rope和cross-attention不太适配，可以试试去掉或者换成一般的pe
