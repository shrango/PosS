...
parser.add_argument('--token_loss_only', default=False, action='store_true')
parser.add_argument('--skip_first_d', type=int, default=0, help="skip the first d depth.")
parser.add_argument('--resume_training', action='store_true', help="load previous optimizer and scheduler.")
...
train_config = {
    ...
    "is_warmup": True if args.disable_warmup is not True else False,
...
if args.token_loss_only:
    train_config["p_w"] = 1.0
    train_config["v_w"] = 0.0
...
from model.cnets_poss_layer import Model
...
if accelerator.is_main_process:
    ...
    wandb.init(project="Position-Specialists", entity="enter your wandb entity here", dir="local wandb dir", config=train_config)
...
def compute_loss(target, target_p, predict, loss_mask):
    ...
    if args.token_loss_only:
        vloss = torch.tensor(0.0, device=predict.device)
    else:
...
if args.ckpt_path is not None: 
    ...
    if not os.path.exists(ea_model_path):
    ...
    if model.layer_num > 1:
        ea_layer_state_dict = model.init_layer_weight(ea_layer_state_dict)
...
if args.resume_training is True and args.ckpt_path is not None: 
    optimizer_path = os.path.join(args.ckpt_path, "optimizer.bin")
    scheduler_path = os.path.join(args.ckpt_path, "scheduler.bin")

    optimizer.load_state_dict(torch.load(optimizer_path))
    if is_warmup:
        scheduler.load_state_dict(torch.load(scheduler_path))

    print(f"load training states from {load_model_path}")
...
model = DDP(unwrapped_model, find_unused_parameters=True)
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            for forward_idx in range(args.skip_first_d):
                with torch.no_grad():
                    predict = model(hidden_states, input_ids, attention_mask, q_hidden_states=q_hidden_states, forward_num=forward_idx)
            ...
            for forward_idx in range(args.skip_first_d, args.forward_num_total):
                predict = model(hidden_states, input_ids, attention_mask, q_hidden_states=q_hidden_states, forward_num=forward_idx)
    ...
    if (epoch + 1) % train_config["save_freq"]:
        ...
        # NOTE: The following validation codes are commented because it sometimes causes blocking.
