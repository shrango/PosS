...
from model.cnets_mlp_full import Model
...
if args.ckpt_path is not None: 
    ...
    for i in range(args.forward_num_total-1):
        ea_layer_state_dict[f'layers.0.extra_mlps.{i}.gate_proj.weight'] = ea_layer_state_dict['layers.0.mlp.gate_proj.weight']
        ea_layer_state_dict[f'layers.0.extra_mlps.{i}.up_proj.weight'] = ea_layer_state_dict['layers.0.mlp.up_proj.weight']
        ea_layer_state_dict[f'layers.0.extra_mlps.{i}.down_proj.weight'] = ea_layer_state_dict['layers.0.mlp.down_proj.weight']
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            if isinstance(model, DDP):
                model.module.layers[0].bridge_weight.data.fill_(1.0)
            else:
                model.layers[0].bridge_weight.data.fill_(1.0)
