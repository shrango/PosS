...
parser.add_argument('--ckpt_path', type=str, required=True, help="you have to provide a ckpt path for further lora training")
...
from model.cnets_manual_attn_lora import Model
...
for layer in model.layers:
    layer.self_attn.initialize_dora_params()
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            if isinstance(model, DDP):
                model.module.layers[0].self_attn.bridge_weight.data.zero_()
            else:
                model.layers[0].self_attn.bridge_weight.data.zero_()
    ...
    if (epoch + 1) % train_config["save_freq"]:
        ...
        #         break
            ...
            # with torch.no_grad():
            #     if batch_idx < 1:
            #         acces = getkacc(model, data, head, max_length=args.forward_num_total)
            #         for i in range(len(acces)):
            #             k_acc[i].append(acces[i])

            #     q_hidden_states = None
            #     for forward_idx in range(args.forward_num_total):
            #         predict = model(data["hidden_states"], input_ids=data["input_ids"], attention_mask=data["attention_mask"], q_hidden_states=q_hidden_states, forward_num=forward_idx)
            #         if q_hidden_states is None:
