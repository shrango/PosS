...
ckpt=/home/jovyan/workspace/project/manual-attn-lora-from-eagle/checkpoint
mkdir -p $ckpt
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --main_process_port 29501 -m --mixed_precision=bf16 train.main_manual_attn_lora \
    --basepath $BasePath \
    --tmpdir $prep \
    --cpdir $ckpt \
