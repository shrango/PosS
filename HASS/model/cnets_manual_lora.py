...
class LoraMLP(nn.Module):
    ...
    def forward(self, x):
        lora_output = self.lora_B(self.lora_A(self.dropout_layer(x))) * self.scaling
        return lora_output
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config, index, forward_num_total):
        ...
        lora_dropout = 0.1
        lora_bias = False
        lora_layers = forward_num_total
        self.lora_mlps = nn.ModuleList([LoraMLP(config.hidden_size, config.hidden_size, lora_rank, lora_alpha, lora_dropout, lora_bias) for _ in range(lora_layers)])

    def require_lora_grad(self):
        for lora_mlp in self.lora_mlps:
            for param in lora_mlp.parameters():
    ...
    def forward(
        ...
        lora_hidden_states = self.lora_mlps[lora_layer](hidden_states) if lora_layer >= 0 else self.bridge_weight * hidden_states
        hidden_states = self.mlp(hidden_states) + lora_hidden_states
        hidden_states = residual + hidden_states
