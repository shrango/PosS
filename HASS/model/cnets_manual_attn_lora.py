...
class DoraLinearLayer(nn.Module):
    def __init__(self, in_feature, out_feature, lora_rank=8, lora_alpha=16, lora_dropout=0.0, lora_bias=False):
        super().__init__()
    ...
    def reset_parameters(self, base_layer):
        ...
        print(f"magnitude starting at {self.magnitude}")
    ...
    def forward(self, x, base_layer, base_result=None):
        ...
        # 解码的时候，一直到mag_norm_scale这一步，都是可以预先计算出来的
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config, lora_layer_num):
        super().__init__()
        self.config = config
        ...
        # lora_layer_num个dora
        self.q_doras = nn.ModuleList([DoraLinearLayer(in_feature=dora_config["in_feature"], out_feature=self.num_heads * self.head_dim, lora_rank=dora_config["lora_rank"], lora_alpha=dora_config["lora_alpha"], lora_dropout=dora_config["lora_dropout"], lora_bias=dora_config["lora_bias"]) for _ in range(lora_layer_num)])

        self.k_doras = nn.ModuleList([DoraLinearLayer(in_feature=dora_config["in_feature"], out_feature=self.num_key_value_heads * self.head_dim, lora_rank=dora_config["lora_rank"], lora_alpha=dora_config["lora_alpha"], lora_dropout=dora_config["lora_dropout"], lora_bias=dora_config["lora_bias"]) for _ in range(lora_layer_num)])
        ...
        # self.q_dora = DoraLinearLayer(
        #     in_feature=dora_config["in_feature"],
        #     out_feature=self.num_heads * self.head_dim,
        #     lora_rank=dora_config["lora_rank"],
        #     lora_alpha=dora_config["lora_alpha"],
        #     lora_dropout=dora_config["lora_dropout"],
        #     lora_bias=dora_config["lora_bias"]
        # )
        # self.k_dora = DoraLinearLayer(
        #     in_feature=dora_config["in_feature"],
        #     out_feature=self.num_key_value_heads * self.head_dim,
        #     lora_rank=dora_config["lora_rank"],
        #     lora_alpha=dora_config["lora_alpha"],
        #     lora_dropout=dora_config["lora_dropout"],
        #     lora_bias=dora_config["lora_bias"]
        # )
        # self.v_dora = DoraLinearLayer(
        #     in_feature=dora_config["in_feature"],
        #     out_feature=self.num_key_value_heads * self.head_dim,
        #     lora_rank=dora_config["lora_rank"],
        #     lora_alpha=dora_config["lora_alpha"],
        ...
        #     lora_bias=dora_config["lora_bias"]
        # )
        # self.o_dora = DoraLinearLayer(
        #     in_feature=self.num_heads * self.head_dim,
        #     out_feature=self.hidden_size,
        #     lora_rank=dora_config["lora_rank"],
        #     lora_alpha=dora_config["lora_alpha"],
        #     lora_dropout=dora_config["lora_dropout"],
        #     lora_bias=dora_config["lora_bias"]
        # )
        self.bridge_weight = nn.Parameter(torch.zeros(1))
    
    def initialize_dora_params(self):
        for lora_layer in self.q_doras:
            lora_layer.reset_parameters(base_layer=self.q_proj)
        for lora_layer in self.k_doras:
            lora_layer.reset_parameters(base_layer=self.k_proj)
        for lora_layer in self.v_doras:
            lora_layer.reset_parameters(base_layer=self.v_proj)
        for lora_layer in self.o_doras:
            lora_layer.reset_parameters(base_layer=self.o_proj)

    def require_lora_grad(self):
        for lora_layer in self.q_doras:
        ...
        for lora_layer in self.o_doras:
            for param in lora_layer.parameters():
                param.requires_grad = True
        self.bridge_weight.requires_grad = True
    ...
    def forward(
            ...
            lora_layer: Optional[int] = -1,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        bsz, q_len, _ = hidden_states.size()

        if self.config.pretraining_tp > 1:
            # 不用管，pretraining_tp默认是1，也不会改
            assert False
            ...
            if forward_num == 1:
                ...
                if lora_layer < 0:
                    query_states_dora = self.bridge_weight * query_states
                else:
                    query_states_dora = self.q_doras[lora_layer](hidden_states, self.q_proj, base_result=query_states)
            else:
                query_states = self.q_proj(q_hidden_states[-1])
                if lora_layer < 0:
                    query_states_dora = self.bridge_weight * query_states
                else:
                    query_states_dora = self.q_doras[lora_layer](q_hidden_states[-1], self.q_proj, base_result=query_states)
            ...
            if lora_layer < 0:
                key_states_dora = self.bridge_weight * key_states
            else:
                key_states_dora = self.k_doras[lora_layer](all_hidden_states, self.k_proj, base_result=key_states)
            key_states = key_states + key_states_dora
            value_states = self.v_proj(all_hidden_states)
            if lora_layer < 0:
                value_states_dora = self.bridge_weight * value_states
            else:
                value_states_dora = self.v_doras[lora_layer](all_hidden_states, self.v_proj, base_result=value_states)
        ...
        if self.config.pretraining_tp > 1:
            ...
            if lora_layer < 0:
                attn_output_dora = self.bridge_weight * attn_output
            else:
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config, index, forward_num_total):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention(config=config, lora_layer_num=forward_num_total)
...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, forward_num_total=3, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0):
        super().__init__()
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayer(config, index, forward_num_total-1) for index in range(config.num_hidden_layers)])
        ...
        for layer in self.layers:
            layer.self_attn.require_lora_grad()
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        ...
                        return module(*inputs, past_key_value, output_attentions, lora_layer=forward_num-1)
