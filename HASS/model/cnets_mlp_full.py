...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config, index, forward_num_total):
        ...
        self.bridge_weight = nn.Parameter(torch.ones(1))
        ...
        self.extra_mlps = nn.ModuleList([LlamaMLP(config) for _ in range(forward_num_total)])

    
    def require_extra_mlp_grad(self):
    ...
    def forward(
        ...
        mlp_hidden_states = self.extra_mlps[lora_layer](hidden_states) if lora_layer >=0 else self.mlp(hidden_states) * self.bridge_weight
        # lora_hidden_states = self.lora_mlps[lora_layer](hidden_states) if lora_layer >= 0 else 0
