...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, forward_num_total=3, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0):
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayer(config, index, forward_num_total) for index in range(config.num_hidden_layers)])
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        ...
                        return module(*inputs, past_key_value, output_attentions, lora_layer=forward_num)
