...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # decoder_layer.self_attn(hidden_states=hidden_states,attention_mask=attention_mask,position_ids=position_ids,past_key_value=past_key_value,output_attentions=output_attentions,use_cache=use_cache,q_hidden_states=q_hidden_states,)
...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0, train_depth=None):
        ...
        self.layers = nn.ModuleList([
            get_peft_model(LlamaDecoderLayer(config, index), lora_config) for index in range(self.train_depth)])
