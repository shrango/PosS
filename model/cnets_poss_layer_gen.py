...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, forward_num_total=3, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0, position_per_layer=1):
        super().__init__()
        ...
        self.layer_num = forward_num_total // self.position_per_layer
        print("self.layer_num:", self.layer_num)

        self.layers = nn.ModuleList([LlamaDecoderLayer(config, index) for index in range(self.layer_num)])
    ...
    def forward(
        ...
        forward_layer = min(forward_layer, self.layer_num-1)
        ...
        decoder_layer = self.layers[forward_layer]
    ...
    @torch.no_grad()
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor, last_accept_length=0, log=False):
        if self.previous_hidden_states is None:
            self.previous_hidden_states = hidden_states
        else:
            self.previous_hidden_states = torch.cat((self.previous_hidden_states, hidden_states), dim=1)
        ...
        if hasattr(self, "stable_kv") and self.stable_kv[0] is not None:
            ...
            first_round = True
        input_length_this_round = position_ids.shape[1]
        # print(f"position_ids A: {position_ids.shape}")
        ...
        # tree_mask = torch.eye(top_k, device=self.embed_tokens.weight.device)[None, None]
        # tree_mask = tree_mask.to(self.embed_tokens.weight.device)
        ...
        for i in range(depth):
            ...
            if hasattr(self, "stable_kv") and self.stable_kv[current_layer] is not None:
                # Three cases for this branch:
                # 1. The first time topK_generate is called, but (i+1)%self.position_per_layer!=0, in this case stable_kv is not generated by it, but by the previous pos, and can be used directly with past_key_values
                # 2. topK_generate is called for the second time, and (i+1)%self.position_per_layer==0, in this case stable_kv should be used to initialize past_key_values
                # 3. topK_generate is called for the second time, and (i+1)%self.position_per_layer!=0, in this case stable_kv is not generated by it, but by the previous pos, and can be used directly with past_key_values
                ...
                if first_round==True:
                    ...
                    # When topK_generate is called for the second time.
                    # Situation 2
                    if (i+1)%self.position_per_layer==0 and layer_num_overflow==False:
                        # The kv_len here may be short, resulting in the result that input_ids[:, kv_len:] and self.stable_kv[current_layer] are correct, but ``mid_hidden_states`` and ``position_ids`` are short, need to be filled
                        # mid_hidden_states is supplemented with self.previous_hidden_states, and position_ids is directly filled with numbers
                        gap_len = input_ids[:, kv_len:].shape[1] - position_ids.shape[0]
                        if gap_len > 0:
                            temp_hidden_states = torch.cat((self.previous_hidden_states[:,-gap_len:,:], mid_hidden_states), dim=1)
                            temp_position_ids = torch.cat((torch.arange(position_ids[0]-gap_len, position_ids[0], device=position_ids.device), position_ids), dim=0)
                        else:
                            temp_hidden_states = mid_hidden_states
                            temp_position_ids = position_ids
                        out_hidden, past_key_values, _ = self(temp_hidden_states, input_ids=input_ids[:, kv_len:],
                                               past_key_values=self.stable_kv[current_layer], position_ids=temp_position_ids, use_cache=True, forward_layer=current_layer)
                    else:
                        # Situation 3
                ...
                if mid_hidden_states.shape[1] < input_ids.shape[1]:
                    temp_hidden_states = torch.cat((self.previous_hidden_states, mid_hidden_states[:,input_length_this_round:,:]), dim=1)
                    temp_position_ids = torch.cat((torch.arange(0, self.previous_hidden_states.shape[1] - input_length_this_round, device=position_ids.device), position_ids), dim=0)
                    # assert temp_hidden_states.shape[1] == temp_position_ids.shape[0] == input_ids.shape[1]
                else:
                    temp_hidden_states = mid_hidden_states
                    temp_position_ids = position_ids
        ...
        if log:
            topk_logit_lists = torch.cat(topk_logits, dim=0).view(-1)
        ...
        if log:
            draft_token_logits = topk_logit_lists[top_scores_index]
            pad_logit = torch.zeros(1).to(draft_token_logits)
            draft_token_logits = torch.cat((pad_logit, draft_token_logits), dim=0)
        ...
        return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, draft_token_logits
