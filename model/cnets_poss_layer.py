...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        align_masks = align_masks.to(query_states.dtype)
...
class Model(nn.Module):
    ...
    def init_layer_weight(self, ea_layer_state_dict):
        last_exist_layer = 0
        for i in range(1, self.layer_num):
            if f'layers.{i}.mlp.gate_proj.weight' not in ea_layer_state_dict:
                print(f"use layer {last_exist_layer}.")
                ea_layer_state_dict[f'layers.{i}.mlp.gate_proj.weight'] = ea_layer_state_dict[f'layers.{last_exist_layer}.mlp.gate_proj.weight']
            else:
                last_exist_layer = i
            if f'layers.{i}.mlp.up_proj.weight' not in ea_layer_state_dict:
                ea_layer_state_dict[f'layers.{i}.mlp.up_proj.weight'] = ea_layer_state_dict[f'layers.{last_exist_layer}.mlp.up_proj.weight']
            else:
                last_exist_layer = i
            if f'layers.{i}.mlp.down_proj.weight' not in ea_layer_state_dict:
                ea_layer_state_dict[f'layers.{i}.mlp.down_proj.weight'] = ea_layer_state_dict[f'layers.{last_exist_layer}.mlp.down_proj.weight']
            else:
                last_exist_layer = i
            if f'layers.{i}.post_attention_layernorm.weight' not in ea_layer_state_dict:
                ea_layer_state_dict[f'layers.{i}.post_attention_layernorm.weight'] = ea_layer_state_dict[f'layers.{last_exist_layer}.post_attention_layernorm.weight']
            else:
                last_exist_layer = i
            if f'layers.{i}.self_attn.k_proj.weight' not in ea_layer_state_dict:
                ea_layer_state_dict[f'layers.{i}.self_attn.k_proj.weight'] = ea_layer_state_dict[f'layers.{last_exist_layer}.self_attn.k_proj.weight']
            else:
                last_exist_layer = i
            if f'layers.{i}.self_attn.o_proj.weight' not in ea_layer_state_dict:
                ea_layer_state_dict[f'layers.{i}.self_attn.o_proj.weight'] = ea_layer_state_dict[f'layers.{last_exist_layer}.self_attn.o_proj.weight']
            else:
                last_exist_layer = i
            if f'layers.{i}.self_attn.q_proj.weight' not in ea_layer_state_dict:
                ea_layer_state_dict[f'layers.{i}.self_attn.q_proj.weight'] = ea_layer_state_dict[f'layers.{last_exist_layer}.self_attn.q_proj.weight']
            else:
                last_exist_layer = i
            if f'layers.{i}.self_attn.v_proj.weight' not in ea_layer_state_dict:
                ea_layer_state_dict[f'layers.{i}.self_attn.v_proj.weight'] = ea_layer_state_dict[f'layers.{last_exist_layer}.self_attn.v_proj.weight']
            else:
                last_exist_layer = i
        return ea_layer_state_dict
    ...
    def forward(
        ...
        decoder_layer = self.layers[forward_num//self.position_per_layer]
